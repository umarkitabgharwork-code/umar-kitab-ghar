...
} from "@/data/specialCourse9thPackages";
  ...
  return (
    <Card className="rounded-2xl border border-[#E8DEC8] bg-[#FFFDF8] shadow-sm">
      <CardContent className="space-y-5 p-6">
        ...
        {pkg.note ? (
          <p className="text-xs text-[#5F7F64]">{pkg.note}</p>
        ) : null}
...
const NinthSpecialCourseFlow = () => {
  ...
  const groups = boardId && boardId !== "other" ? getGroupsForBoard(boardId) : [];
  const trackPackages =
    boardId && boardId !== "other" && selectedGroup
      ? getTracksForGroup(boardId, selectedGroup)
      : [];
  ...
  const addSpecialPackageToCart = (opts: {
    ...
    addItem({
      id: `special-course-9th-${opts.cartIdSuffix}-${crypto.randomUUID()}`,
      name: opts.packageName,
      price: 0,
      category: `${opts.board} - ${NINTH_CLASS_LABEL}`,
      type: "course",
      schoolName: opts.board,
      className: NINTH_CLASS_LABEL,
  ...
  const handleOtherBoardAdd = (goCheckout: boolean) => {
    ...
    const packageName = "Other Board Course";
    addSpecialPackageToCart({
      packageName,
      board: "Other Board",
  ...
  if (boardId === "other") {
    return (
      <div className="space-y-6 animate-fade-in">
        <SelectionSummary classLabel={NINTH_CLASS_LABEL} boardLabel={boardLabel} />
        <Card className="rounded-2xl border border-[#E8DEC8] bg-[#FFFDF8] shadow-sm">
          <CardContent className="space-y-5 p-6">
            <div>
